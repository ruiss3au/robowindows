#!/usr/bin/env bash
set -euo pipefail
[[ $# == 3 && -f $3 ]] || { echo "Usage: summarize-presentation.sh normal|dynamic 0|1 LOG" >&2; exit 2; }
case "$1" in normal) decoder=Normal ;; dynamic) decoder=DynRec ;; *) exit 2 ;; esac
[[ $2 == 0 || $2 == 1 ]] || exit 2
awk -v decoder="$decoder" -v policy="$2" '
...
/RoboWindowsPresentation: PASS timer/ { timer=1 }
/RoboWindowsPresentation: FAIL|Fatal signal|FATAL EXCEPTION/ { fail("Fixture failed") }
END {
  if (!timer || ms < 100000 || !posts || !produced || !consumed) fail("Incomplete timer/audio capture")
