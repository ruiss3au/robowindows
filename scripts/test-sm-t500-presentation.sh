...
core=${1:-}
policy=${2:-}
[[ $core == normal || $core == dynamic ]] && [[ $policy == 0 || $policy == 1 ]] || {
  echo "Usage: bash scripts/test-sm-t500-presentation.sh normal|dynamic 0|1" >&2; exit 2;
}
...
output=$(mktemp -d "$repo_dir/artifacts/presentation-${core}-${policy}-XXXXXX")
...
echo "Starting disposable $core presentation=$policy; keep the tablet unlocked and foregrounded."
"${RW_ADB_TARGET[@]}" shell am start -W -n "$package/org.robowindows.app.MainActivity" \
  --ez robowindows.testPresentationWorkload true --es core "$core" --ei presentation "$policy" >/dev/null
for attempt in {1..150}; do
  ...
  if rg -q 'RoboWindowsPresentation: (PASS|FAIL)' "$output/telemetry.log"; then
    ...
    bash "$repo_dir/scripts/summarize-presentation.sh" "$core" "$policy" "$output/telemetry.log" | tee "$output/summary.txt"
    exit
  fi
