...
bash "$repo_dir/scripts/summarize-presentation.sh" normal 1 "$work_dir/valid.log" | grep -q 'quality=pass'
...
bash "$repo_dir/scripts/summarize-presentation.sh" normal 1 "$work_dir/stress.log" stress | grep -q 'quality=pass'
